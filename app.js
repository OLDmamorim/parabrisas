...
/* =========== PATCH: Exportar Excel com Modal (não mexe no resto) =========== */
(function () {
  // Carrega SheetJS se ainda não estiver presente
  ...
  function filterByDateRange(rows, startYMD, endYMD) {
    ...
    const start = startYMD ? new Date(startYMD + 'T00:00:00') : null;
    const end   = endYMD   ? new Date(endYMD   + 'T23:59:59') : null;
    return rows.filter(r => {
      const dt = parseAnyDate(r.timestamp);
      if (!dt) return false;
      if (start && dt < start) return false;
  ...
  // Memória local (último período e checkbox)
  const MEM_S = 'export_start_ymd';
  const MEM_E = 'export_end_ymd';
  const MEM_U = 'export_use_search';
  function savePrefs(s, e, u) { try {
    localStorage.setItem(MEM_S, s || '');
    localStorage.setItem(MEM_E, e || '');
    localStorage.setItem(MEM_U, u ? '1' : '0');
  } catch(_){} }
  function loadPrefs() { try {
    return { s: localStorage.getItem(MEM_S) || '', e: localStorage.getItem(MEM_E) || '', u: (localStorage.getItem(MEM_U) || '1') === '1' };
  } catch(_) { return { s:'', e:'', u:true }; } }

  // Converte dados para linhas do Excel
  ...
  async function exportExcelWithData(data, filename) {
    await loadSheetJS();
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Registos');
    XLSX.writeFile(wb, filename || `expressglass_${new Date().toISOString().split('T')[0]}.xlsx`);
    try { showToast && showToast('Excel exportado!', 'success'); } catch(_){}
  }

  // Cria o modal on-the-fly (não mexe no HTML)
  function ensureExportModal() {
    if (document.getElementById('exportModal')) return;
    const wrap = document.createElement('div');
    wrap.id = 'exportModal';
    wrap.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.7);display:none;align-items:center;justify-content:center;z-index:1200;backdrop-filter:blur(6px);padding:16px;';
    wrap.innerHTML = `
      <div style="width:100%;max-width:520px;background:#0f172a;color:#e5e7eb;border:1px solid #1f2937;border-radius:16px;box-shadow:0 20px 40px rgba(0,0,0,.35);display:flex;flex-direction:column;overflow:hidden;">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 20px;border-bottom:1px solid #1f2937;">
          <h3 style="margin:0;font-size:18px;color:#22d3ee">📤 Exportar para Excel</h3>
          <button id="exportClose" title="Fechar" style="background:none;border:0;color:#a3aed0;font-size:24px;cursor:pointer;line-height:1;border-radius:8px;padding:2px 6px;">×</button>
        </div>
        <div style="padding:16px 20px;display:grid;gap:12px">
          <label>Data início <input id="exportStart" type="date" style="margin-top:6px;background:rgba(255,255,255,.06);border:1px solid #1f2937;color:#e5e7eb;border-radius:10px;padding:10px 12px;font-size:14px;"></label>
          <label>Data fim <input id="exportEnd" type="date" style="margin-top:6px;background:rgba(255,255,255,.06);border:1px solid #1f2937;color:#e5e7eb;border-radius:10px;padding:10px 12px;font-size:14px;"></label>
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:2px;">
            <button type="button" id="presetHoje"  style="background:#0ea5e9;border:none;color:white;padding:6px 10px;border-radius:999px;cursor:pointer;font-weight:700;">Hoje</button>
            <button type="button" id="preset7"     style="background:#0ea5e9;border:none;color:white;padding:6px 10px;border-radius:999px;cursor:pointer;font-weight:700;">Últimos 7 dias</button>
            <button type="button" id="presetMes"   style="background:#0ea5e9;border:none;color:white;padding:6px 10px;border-radius:999px;cursor:pointer;font-weight:700;">Este mês</button>
            <button type="button" id="presetTodos" style="background:#0ea5e9;border:none;color:white;padding:6px 10px;border-radius:999px;cursor:pointer;font-weight:700;">Todos</button>
          </div>
          <label style="display:flex;align-items:center;gap:8px;"><input id="exportUseSearch" type="checkbox" checked> Usar também o filtro de pesquisa atual</label>
        </div>
        <div style="display:flex;gap:10px;justify-content:flex-end;padding:16px 20px;border-top:1px solid #1f2937;">
          <button id="exportCancel"  style="background:#6b7280;color:#fff;border:none;padding:10px 16px;border-radius:10px;cursor:pointer;">Cancelar</button>
          <button id="exportConfirm" style="background:#06b6d4;color:#fff;border:none;padding:10px 16px;border-radius:10px;cursor:pointer;font-weight:700;">Exportar</button>
        </div>
      </div>`;
  ...
  function openExportModal() {
    ...
    const startEl = document.getElementById('exportStart');
    const endEl   = document.getElementById('exportEnd');
    const useEl   = document.getElementById('exportUseSearch');

    // Prefill com memória + defaults
    const memo = loadPrefs();
    if (memo.s) startEl.value = memo.s;
    if (memo.e) endEl.value   = memo.e;
    if (useEl)  useEl.checked = memo.u;

    const today = new Date();
    if (!endEl.value)   endEl.value   = fmtYMD(today);
    if (!startEl.value) startEl.value = fmtYMD(new Date(today.getTime() - 29*24*3600*1000));

    // Presets
    const setDates = (s, e) => { startEl.value = s || ''; endEl.value = e || ''; };
    document.getElementById('presetHoje').onclick  = () => { const t=new Date(); const ymd=fmtYMD(t); setDates(ymd, ymd); };
    document.getElementById('preset7').onclick     = () => { const t=new Date(); const s=new Date(t.getTime()-6*24*3600*1000); setDates(fmtYMD(s), fmtYMD(t)); };
    document.getElementById('presetMes').onclick   = () => { const t=new Date(); setDates(fmtYMD(firstDayOfMonth(t)), fmtYMD(t)); };
    document.getElementById('presetTodos').onclick = () => setDates('', '');
    ...
    document.getElementById('exportConfirm').onclick = async () => {
      ...
      const e = endEl.value   || '';
      const useFilter = !!(useEl && useEl.checked);
      savePrefs(s, e, useFilter);

      const base = (useFilter && typeof FILTERED_RESULTS !== 'undefined' && FILTERED_RESULTS.length)
        ? FILTERED_RESULTS : (Array.isArray(RESULTS) ? RESULTS : []);
      const ranged = filterByDateRange(base, s, e);

      const fname = `expressglass_${(s||'todos')}_a_${(e||'todos')}.xlsx`;
      await exportExcelWithData(ranged, fname);
      close();
    };
  ...
  // Deixa global para o HTML e para compatibilidade
  window.openExportModal = openExportModal;
  window.exportCSV = openExportModal; // chamadas antigas passam a abrir o modal

  // Garanta que o botão aponta para Excel
  document.addEventListener('DOMContentLoaded', () => {
    const btn = document.getElementById('btnExport');
    if (btn) {
      btn.innerHTML = '📊 Exportar Excel';
      btn.removeAttribute('onclick'); // evitamos chamadas antigas
