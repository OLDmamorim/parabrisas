...
function showEurocodeValidationModal(ocrText, filename, source, vehicle) {
  ...
  // >>> Novo: decide por visibilidade real do mobileView
  const mv = document.getElementById('mobileView');
  const usingMobileView = !!mv && window.getComputedStyle(mv).display !== 'none';
  ...
  content.innerHTML = `
    ...
    ${usingMobileView ? `
      <div class="euro-modal-actions" style="display:flex; gap:10px; justify-content:center;">
