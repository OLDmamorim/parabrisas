...
async function runOCR(payload) {
  ...
  try {
    ...
    // 2ª tentativa (se vier vazio, envia a dataURL completa)
    if (!text) {
...
// =========================
// Processar imagem (downscale + fallback de leitura)
async function processImage(file) {
  ...
  try {
    // Lê como dataURL completa (fica com as duas variantes à mão)
    ...
    // Tenta primeiro com imagem reduzida (evita timeouts no OCR)
    let base64Reduced = null;
    try {
      base64Reduced = await downscaleImageToBase64(file, 1800, 0.8);
    } catch (_) {
      // se falhar o downscale, seguimos com o original
    }

    // 1ª tentativa: base64 reduzido (ou base64 original)
    let ocrText = await runOCR(base64Reduced || dataURL);
    // 2ª tentativa: se não veio nada, envia explicitamente a dataURL original
