...
export const handler = async (event) => {
  ...
  try {
    ...
    const { text = '', eurocode = '', filename = '', source = '', brand: brandFromClient = '' } =
      JSON.parse(event.body || '{}');

    // === Detectar marca no backend se vier vazia (redundância segura) ===
    const brand = brandFromClient || detectBrandFromText(text) || null;

    // INSERT na tabela real (nota: euro_validado + ts)
...
/* ---------- Brand detection (mesma lógica do app.js) ---------- */
