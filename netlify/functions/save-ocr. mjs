...
export const handler = async (event) => {
  ...
  try {
    ...
    const { ts, text, filename, source } = JSON.parse(event.body || '{}');
    if (!text && !filename) {
    ...
    const rows = await sql/*sql*/`
      insert into ocr_results (ts, text, filename, source, ip)
      values (${ts ? new Date(ts) : new Date()}, ${text}, ${filename}, ${source}, ${ip})
